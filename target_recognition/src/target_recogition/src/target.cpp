...
#include <chrono>
// 构造函数  初步筛选装甲板并加入数组
armorDetector::armorDetector(const std::vector<std::vector<cv::Point>> &contours)
{
...
// 两个装甲板面积相差越小得分越高
int armorDetector::AreaCout(cv::RotatedRect &a, cv::RotatedRect &b)
{
    int cout = 0;
    int area[2];
...
// 两个装甲板角度差越小得分越高
int armorDetector::AngleCout(cv::RotatedRect &a, cv::RotatedRect &b)
{
    int cout = 0;
...
// 两个装甲板越水平得分越高
int armorDetector::DistanceCout(cv::RotatedRect &leftRect, cv::RotatedRect &rightRect)
{
    int level = 0;
...
// 排除非匹配装甲板
bool armorDetector::isArmor(cv::RotatedRect &a, cv::RotatedRect &b)
{
    if (std::abs(a.center.y - b.center.y) < plate.max_height_difference)
        return true;
    else
        return false;
}
