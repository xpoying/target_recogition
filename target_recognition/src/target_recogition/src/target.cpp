#include"Target.h"
    ...
    Armor::Armor(const std::vector<std::vector<cv::Point>> &contours)
    {
        for (int i = 0; i < contours.size(); i++)
        {
            ...
            if (minRects.size.width * 1.5 < minRects.size.height)
            {
    ...
    //两个装甲板距离越近得分越高
    int Armor::DistanceCout(cv::RotatedRect &a, cv::RotatedRect &b)
    {
        int cout = 0;
        ...
        {cout = cout + 8;}
        else if (std::abs(a.center.x - b.center.x) < 200)
        {cout = cout + 6;}
        else if (std::abs(a.center.x - b.center.x) < 400)
        {cout = cout + 4;}
        else{}
        return cout;
    }
    ...
    {
        if (std::abs(a.center.y - b.center.y) < 10)return true;
        else return false;
    }
...
void DrawAmrorPlate(cv::Mat &src,std::vector<cv::RotatedRect> &AmrorPlate)
{
 ...
 if (AmrorPlate.size() < 2) {
   ...
   cv::RotatedRect left = AmrorPlate[0];
    ...
    left.points(leftxy);
    cv::Point a1 = (leftxy[1] + leftxy[2]) / 2;
    cv::Point a2 = (leftxy[0] + leftxy[3]) / 2;
    cv::Point b1 = (rightxy[1] + rightxy[2]) / 2;
    cv::Point b2 = (rightxy[0] + rightxy[3]) / 2;
